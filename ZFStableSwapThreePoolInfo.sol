...
contract ZFStableSwapThreePoolInfo {
    ...
    function calc_coins_amount(address _swap, uint256 _amount) external view returns (uint256[N_COINS] memory) {
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            uint256 value = (IZFStableSwap(_swap).balances(i) * _amount) / total_supply;
            amounts[i] = value;
        }
        return amounts;
    }
    ...
    function get_add_liquidity_mint_amount(address _swap, uint256[N_COINS] memory amounts)
        ...
        returns (uint256)
    {
        IZFStableSwap swap = IZFStableSwap(_swap);
        ...
        returns (uint256[N_COINS] memory liquidityFee)
    {
        IZFStableSwap swap = IZFStableSwap(_swap);
        uint256 _fee = (swap.fee() * N_COINS) / (4 * (N_COINS - 1));
        uint256 _protocol_fee = swap.protocol_fee();
        uint256 amp = swap.A();
        ...
        if (token_supply > 0) {
            ...
            for (uint256 i = 0; i < N_COINS; i++) {
                ...
                liquidityFee[i] = ((coinFee * _protocol_fee) / FEE_DENOMINATOR);
            }
        }
    }
    ...
    function get_exchange_fee(
        ...
        uint256 j,
        uint256 dx
    ) external view returns (uint256 exFee, uint256 exProtocolFee) {
        IZFStableSwap swap = IZFStableSwap(_swap);
    ...
    function get_remove_liquidity_one_coin_fee(
        ...
        protocolFee = (dy_fee * swap.protocol_fee()) / FEE_DENOMINATOR;
    }
